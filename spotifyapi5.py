...
class App(QMainWindow):
    ...
    def initUI(self):
        ...
        self.platform_combo.currentIndexChanged.connect(self.checkCredentials)
    ...
    def checkCredentials(self):
        ...
        creds_filename = f'{platform.lower().replace(" ", "_")}_creds.json'
        
        if os.path.exists(creds_filename):
            with open(creds_filename, 'r') as file:
                creds = json.load(file)
                self.api_input.setText(creds.get('client_id', ''))
                self.secret_input.setText(creds.get('client_secret', ''))
                self.api_label.hide()
                self.api_input.hide()
